...
class PFLDLoss(nn.Module):
    ...
    def forward(self, attribute_gt, landmark_gt, euler_angle_gt, angle, \
        ...
        weight_angle = torch.sum(1 - torch.cos(angle - euler_angle_gt), dim=1)   # default : axis=1
        ...
        if self.use_gpu:
            mat_ratio = mat_ratio.cuda(0)
        weight_attribute = torch.sum(attributes_w_n.mul(mat_ratio), dim=1)
